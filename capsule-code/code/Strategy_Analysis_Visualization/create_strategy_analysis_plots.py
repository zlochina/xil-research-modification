...
sns.set(style='ticks', palette=customPalette)
sns.despine()
# rc('text', usetex=True)
...
parser.add_argument('--fp-cams', type=str, required=True,
                    help='please specifiy the path to the data folder containing the npy files of the gradcams')
parser.add_argument('--fp-testsplit', type=str, required=True,
                    help='please specifiy the full path including name to the txt test split file')
...
def visualize_scatter_with_images_clusters(X_2d_data, images, fp, cluster_ids, figsize=(45,45), image_zoom=1,
	...
	for i in np.arange(0, k):
		...
		scatter = plt.scatter(X_2d_data[ids, 0], X_2d_data[ids, 1], s=20000, alpha=0.85)
	if legend:
	...
	ax.autoscale()
	ax.axis('off')
	fig.savefig(fp, dpi=300, bbox_inches='tight', pad_inches=0)
	plt.close()
...
def compute_affinity_matrix(imgs, n_nn, image_type, loss_type):
	"""
	...
	if 'rgb' in image_type:
		...
		connectivity = kneighbors_graph(fourier_imgs, metric='euclidean', n_neighbors=n_nn, n_jobs=NJOBS)
...
def get_filenames(image_type, loss_type):
	...
	if 'rgb' in image_type:
		if 'default' in loss_type:
			fp_save = "results/strategy_analysis_interim_results/affinity_matrices/rgb_default_nclusters_affinity.npy"
		elif 'rrr' in loss_type:
			fp_save = "results/strategy_analysis_interim_results/affinity_matrices/rgb_rrr_nclusters_affinity.npy"
	elif 'hs' in image_type:
		if 'default' in loss_type:
			fp_save = "results/strategy_analysis_interim_results/affinity_matrices/hs_default_nclusters_affinity.npy"
		elif 'rrr' in loss_type:
			fp_save = "results/strategy_analysis_interim_results/affinity_matrices/hs_rrr_nclusters_affinity.npy"

	# list of train and test sample ids for corresponding cv run
	with open(args.fp_testsplit) as f:
	...
	for i, sample_id in enumerate(test_samples):
		...
		filenames.append(glob.glob(f"{args.fp_cams}{sample_id}*.npy")[0])
		test_ids.append(i)
...
def compute_save_affinity_matrices():
	...
	dir_name = "results/strategy_analysis_interim_results/affinity_matrices/"
	try:
	...
	affinity_matrix = compute_affinity_matrix(cams_downscaled, n_nn, image_type, loss_type)
...
def create_save_image_embedded_tsne_plots():
	...
	dir_names = ["results/strategy_analysis_interim_results/cluster_assignments/figures"]
	...
	with open(f"results/strategy_analysis_interim_results/cluster_assignments/{image_type}_{loss_type}.txt", "w") as output:
	...
	if loss_type == 'default':
		fp_save = f"results/cnn_{image_type}/paper_plots/fig3/strategy_analysis/{fp_save_ext}_image_embedded_tsne_colored.png"
	elif loss_type == 'rrr':
		fp_save = f"results/cnn_{image_type}/paper_plots/fig4/strategy_analysis/{fp_save_ext}_image_embedded_tsne_colored.png"
...
def create_embedded_tsne_classificationrate_plots():
	...
	print("------------------------------------------------")
	# create the directory for the figures
	dir_name = "results/strategy_analysis_interim_results/cluster_assignments/figures/"
	try:
		os.makedirs(dir_name)
		print("Creating directory: ", dir_name)
	except OSError as e:
	...
	if loss_type == 'default':
		fp_save = f"results/cnn_{image_type}/paper_plots/fig3/strategy_analysis/{fp_save_ext}_image_embedded_tsne_classrates.png"
	elif loss_type == 'rrr':
		fp_save = f"results/cnn_{image_type}/paper_plots/fig4/strategy_analysis/{fp_save_ext}_image_embedded_tsne_classrates.png"
	fig.savefig(fp_save, dpi=300, bbox_inches='tight')
