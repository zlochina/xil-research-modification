...
class GradCam:
    ...
    def generate_cam_3D(self, input_image, target_class=None):
        ...
        if torch.cuda.is_available():
            device = "cuda"
        one_hot_output = (
            torch.FloatTensor(1, model_output.size()[-1]).zero_().to(device)
        )
        one_hot_output[0][target_class] = 1
        # Zero grads
        self.model.features.zero_grad()
        ...
        guided_gradients = self.extractor.gradients.data.cpu().numpy()[
            0
        ]  # CDHW e.g. 128,4,13,13
        # Get convolution outputs
        target = conv_output.data.cpu().numpy()[0]
        # Get weights from gradients
        weights = np.mean(
            guided_gradients, axis=(2, 3)
        )  # Take averages for each gradient
        # Create empty numpy array for cam
        cam = np.ones(target.shape[1:], dtype=np.float32)
        # Multiply each weight with its conv output and then, sum
        ...
        for idx in range(cam.shape[0]):
            cam_tmp = cam[idx]
            cam_resized = Image.fromarray(cam_tmp).resize(
                (input_image.shape[3], input_image.shape[4]), Image.ANTIALIAS
            )
            cams_resized.append(
                ((np.uint8(cam_resized) / 255) * (tmp_max_cam - tmp_min_cam))
                + tmp_min_cam
            )
        ...
        return (
            cam,
            input_image.shape[2] // len(cams_resized),
            (spectral_cam_mean, spectral_cam_std, spectral_cam_median),
        )
