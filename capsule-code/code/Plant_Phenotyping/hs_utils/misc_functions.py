...
def save_class_activation_images3D(
    org_img,
    activation_map,
    file_name,
    samples,
    spectral_activations,
    orig_wavelength,
    wv_plot=True,
):
    """
    ...
    if wv_plot:
        fig, axs = plt.subplots(
            ncols=activation_map.shape[0] + 1, nrows=2, figsize=(12, 20)
        )
        axs = axs.flatten()
        ...
        axs1_twin.plot(
            [
                wavelength_area - wavelength_area / 2,
        ...
        fig, axs = plt.subplots(
            ncols=activation_map.shape[0] + 1, nrows=1, figsize=(12, 20)
        )
        axs_next = axs
    ...
    for idx, ax in enumerate(axs_next):
        ...
        if idx != 0:
            org_img_edged = preprocessing.scale(
                np.array(org_img, dtype=float)[:, :, 1] / 255
            )
...
def save_class_activation_images2D(
    org_img, activation_map, dir_name, file_name, true_class, pred_class
):
    """
        Saves cam activation map and activation map on the original image
