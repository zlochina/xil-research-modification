...
class LeafDataset(Dataset):
    def __init__(self, data_path, mode='train',
                 ...
                 rgb=False, use_downsampled=True):
        ###
        ...
        if self.use_downsampled:
            self.step_wavelength = 1  # NOTE: only works with already downsampled data
            self.clip_wavelength = 400  # NOTE: only works with already downsampled data
        else:
    ...
    def split_data_by_ids(self, data, labels, split):
        train_ids = np.loadtxt("code/Plant_Phenotyping/hs_dataset_splits/cv{}/train.txt".format(split), dtype=str).tolist()
        test_ids = np.loadtxt("code/Plant_Phenotyping/hs_dataset_splits/cv{}/val.txt".format(split), dtype=str).tolist()
    ...
    def get_samples(self, samples, verbose=False):
        ...
        if self.use_downsampled:
            view_channels_ = [c // 5 for c in view_channels_vnir]
    ...
    def _load_background_masks(self, data_path="code/Plant_Phenotyping/background_labels"):
        import glob
    ...
    def _load_data_single(self, memmap_path, single_image=False, use_labels=None, use_files=None, n_shots=-1):
        ...
        if self.use_downsampled:
            memmap_shape = (memmap_shape[0], memmap_shape[1], memmap_shape[2], 64)
