...
parser.add_argument(
    "--seed", default=17, type=int, help="seed for initializing training. "
)
parser.add_argument("--gpus", default=None, type=str, help="GPU id to use.")
parser.add_argument(
...
parser.add_argument("--mask", default=0, type=int, help="")
parser.add_argument("--mask_eval", default=0, type=int, help="")
parser.add_argument("--l2_grads", default=8, type=int, help="")
parser.add_argument("--cv_splits", default=1, type=int, help="")
parser.add_argument("--cv_current_split", default=0, type=int, help="")
...
def main_worker(gpu, args):
    ...
    if args.resume:
        ...
        if os.path.isfile(args.correct_rwr):
            print(
                "=> loading checkpoint of right for the wrong reasons model '{}'".format(
                    args.correct_rwr
                )
            )
            ...
            print(
                "=> loaded checkpoint '{}' (epoch {})".format(
                    args.correct_rwr, checkpoint["epoch"]
                )
            )
        else:
    ...
    # set process name
    procname = PytorchProcessName(
        args.epochs, name="ML Deepplant %s" % ("hsi classification")
    )
...
def train(train_loader, model, criterion, optimizers, schedulers, epoch, args):
    ...
    for i, optimizer in enumerate(optimizers):
        writer.add_scalar(
            "lr/{}".format(i + 1), optimizer.param_groups[0]["lr"], epoch + 1
        )
    ...
    for batch_idx, (x, y, _) in enumerate(train_loader):
        ...
        output = model(input)
        if train_loader.dataset.mask_background == 2:
            loss, loss_ra, loss_rrr = rrr_loss_function(
                A=A,
...
def validate(val_loader, model, criterion, epoch, args):
    ...
    top1_balanced = AverageBalancedAccuracyMeter("BalancedAcc@1", ":6.2f")
    progress = ProgressMeter(
        len(val_loader), batch_time, losses, top1, top1_balanced, prefix="Test: "
    )
    ...
    with torch.no_grad():
        ...
        if args.evaluate:
            cm_cnt = confusion_matrix(
                y_pred=y_pred, y_true=y_true, labels=np.unique(y_true)
            )
            ...
            print(
                " *Total Acc@1 {top1.avg:.2f} | Acc@1 Balanced {top1_b.avg:.2f} ".format(
                    top1=top1, top1_b=top1_balanced
                )
            )
            print("-- " * 42)
            print("-- END --")
...
def grad_cam(data_loader, model, args, save_path_suffix):
    ...
    save_dir = os.path.join(
        os.path.dirname(args.resume),
        "gradcam_" + str(os.path.basename(args.resume).split(".")[0]),
    )
    save_dir = os.path.join(save_dir, save_path_suffix)
    os.makedirs(save_dir, exist_ok=True)
    os.makedirs(os.path.join(save_dir, "npy"), exist_ok=True)
    ...
    for i, (input, y, _) in enumerate(tqdm(data_loader)):
        ...
        for idx_x, x in enumerate(input):
            ...
            cam, num_channels_upscaling, spectral_cam = grad_cam_res.generate_cam_3D(
                x.unsqueeze(0), [y_pred]
            )
            ...
            save_class_activation_images3D(
                orig_img,
                cam,
                os.path.join(
...
def getPredAndTarget(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        pred = pred.t()
        return (
            pred.view(-1).detach().cpu().numpy().tolist(),
            target.view(-1).detach().cpu().numpy().tolist(),
        )
