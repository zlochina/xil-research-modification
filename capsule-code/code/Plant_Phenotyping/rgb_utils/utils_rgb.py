...
def imread_from_fp_rescale_rotate_flatten(
    fplist, y=np.array([]), rescale_size=None, n_rot_per_img=0
):
    """
    ...
    if rescale_size is not None:
        data_flatten = np.zeros(
            (fplist.size * n_rot_per_img, rescale_size * rescale_size * 3)
        )
    else:
        ex_img = Image.open(fplist[0])
        data_flatten = np.zeros(
            (fplist.size * n_rot_per_img, ex_img.size[1] * ex_img.size[0] * 3)
        )
    ...
    if y.size is not 0:
        return data_flatten, y
    else:
        return data_flatten


def imread_from_fp_rescale_rotate_flatten_returnmasks(
    fplist,
    fp_mask,
    y=np.array([]),
    rescale_size=None,
    ...
    if rescale_size is not None:
        ...
        masks_flatten = np.zeros(
            (fplist.size * n_rot_per_img, ex_img.size[1] * ex_img.size[0])
        )
    ...
    for idx in np.arange(fplist.size * n_rot_per_img):
        ...
        if not rrr:
            print(
                "FYI: The images are being cut out!!!, the masks are being returned as well"
            )
        ...
        img = Image.fromarray(
            img
        )  # very hacky, pil objects do not support item assignment
    ...
    return data_flatten, masks_flatten, y
