    ...
    return torch.tensor(
        ndimage.zoom(
            ...
            (
                1,
                1,
                output.shape[-2] / mask.shape[-2],
                output.shape[-1] / mask.shape[-1],
            ),
        )
    )
...
def rrr_loss_function(
    A, X, y, logits, criterion, class_weights, l2_grads=1000, reduce_func=torch.sum
):
    log_prob_ys = log_softmax(logits)
    right_answer_loss = criterion(log_prob_ys, y)

    gradXes = torch.autograd.grad(
        log_prob_ys, X, torch.ones_like(log_prob_ys).cuda(), create_graph=True
    )[0]
    ...
    with torch.no_grad():
        ...
        class_weights_batch[class_weights_batch == 0] = 1.0 - class_weights[0]
        class_weights_batch[class_weights_batch == 1] = 1.0 - class_weights[1]
