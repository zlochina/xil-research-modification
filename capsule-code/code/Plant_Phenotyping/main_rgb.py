...
def gen_cams():
    """
    Generate the gradcams of all test data given the filepath to a checkpoint, args.fp_ckpt.
    """
    print("\n----------------------------------------------------------------\n")
    print("\nGenerating GradCAMS \n")
    ...
    filenames_allfiles = filenames_allfiles[test_ids]
    y = y[test_ids]
    ...
    try:
        os.makedirs(fp_cvrun)
        print("Creating directory: ", fp_cvrun)
    except OSError as e:
...
def run_training():
    ...
    if args.rrr:
        ...
        train_tensors = train_tensors + (torch.tensor(masks[train_ids]),)
...
def run_test():
    ...
    print(f"\nTest accuracy of cv run {args.cv_run}: {test_acc:.4f}\n")
    print("\n----------------------------------------------------------------\n")
