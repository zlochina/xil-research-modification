...
def word_table(
    columns, headers, cutoff=None, startafter=None, style="table-layout:fixed", **kwargs
):
...
def email_html(email, row, title=None, swatches=True, power=2):
    ...
    for wt, w in poswts:
        email = re.sub(
            r"([^a-zA-Z0-9]){}([^a-zA-Z0-9])".format(w),
            "\\1{}\\2".format(
                "<span style='background: rgba(155,217,230,{});'>{}</span>".format(
                    (wt / maxwt) ** power, w
                )
            ),
            email,
    ...
    for wt, w in negwts:
        email = re.sub(
            r"([^a-zA-Z0-9]){}([^a-zA-Z0-9])".format(w),
            "\\1{}\\2".format(
                "<span style='background: rgba(238,125,50,{})'>{}</span>".format(
                    (-wt / maxwt) ** power, w
                )
            ),
            email,
    ...
    swatch2 = ""
    if swatches:
        swatch1 = '<span style="background: rgb(155,217,230); font-family:monospace; padding: 0 5px; margin-left: 5px;">+{}</span>'.format(
            pred
        )
