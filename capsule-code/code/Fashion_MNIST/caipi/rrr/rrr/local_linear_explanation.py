                ...
                for i, coef in enumerate(listwrap(coefs[y][x])):
                    if abs(coef) > maxcoef * cutoff:
                        ...
                        plt.scatter(
                            ...
                            s=size,
                            lw=lw,
                            alpha=(abs(coef) / maxcoef)
                            ** pwr,  # make opacity relative to magnitude
                            c=colors[int(np.sign(coef))],  # make color
                            marker=markers[int(np.sign(coef))],
                        )  # and marker depend on sign


def explanation_grid(
    explanations, imgshape, length=None, gridshape=None, pad=0.1, **kwargs
):
    if len(imgshape) == 2:
        l, l2 = imgshape
        assert l == l2
...
def image_grid(images, imgshape, length=None, **kwargs):
    fauxpls = [
        LocalLinearExplanation(images[i], 0, np.zeros_like(images[i]))
