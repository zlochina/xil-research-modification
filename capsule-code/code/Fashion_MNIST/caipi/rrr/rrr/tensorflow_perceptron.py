...
class TensorflowPerceptron:
    def loss_function(self, l2_grads=1000, l1_grads=0, l2_params=0.0001):
        ...
        small_params_loss = l2_params * tf.add_n(
            [tf.nn.l2_loss(p) for p in self.W + self.b]
        )

        return right_answer_loss + right_reason_loss + small_params_loss

    def optimizer(
        self, l2_grads=1000, l1_grads=0, l2_params=0.0001, learning_rate=0.001
    ):
        optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate)
        return optimizer.minimize(
            self.loss_function(
                l2_grads=l2_grads, l1_grads=l1_grads, l2_params=l2_params
            )
        )
    ...
    def fit(
        ...
        for i in range(1, len(self.layer_sizes)):
            self.W.append(
                tf.Variable(
                    tf.random_normal(self.layer_sizes[i - 1 : i + 1], stddev=weight_sd),
                    name="W{}".format(i),
                )
            )
            self.b.append(
                tf.Variable(
                    tf.random_normal([self.layer_sizes[i]], stddev=weight_sd),
                    name="b{}".format(i),
                )
            )
        for i, activation in enumerate(
            [nonlinearity for _ in hidden_layers] + [to_logprob]
        ):
            self.L.append(
                activation(tf.add(tf.matmul(self.L[i], self.W[i]), self.b[i]))
            )
        ...
        with tf.Session() as sess:
            ...
            for i in range(num_epochs * num_batches):
                idx = slice(
                    (i % num_batches) * batch_size, ((i % num_batches) + 1) * batch_size
                )
                sess.run(
                    optimizer,
                    feed_dict={self.X: X[idx], self.y: y[idx], self.A: A[idx]},
                )
