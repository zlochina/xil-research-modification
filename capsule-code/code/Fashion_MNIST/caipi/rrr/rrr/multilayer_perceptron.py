...
def init_random_params(scale, layer_sizes, rs=npr):
    return [
        (scale * rs.randn(m, n), scale * rs.randn(n))  # weight matrix  # bias vector
    ...
    def fit(
        ...
        **input_grad_kwargs
        ...
        def objective(params, iteration):
            ...
            if normalize:
                sumA = max(1.0, float(Ai.sum()))
                lenX = max(1.0, float(len(Xi)))
            else:
                sumA = 1.0
                lenX = 1.0
            ...
            if verbose and verbose(iteration):
                print(
                    "Iteration={}, crossentropy={}, rightreasons={}, smallparams={}, sumA={}, lenX={}".format(
                        iteration,
                        crossentropy.value,
                        rightreasons.value,
                        smallparams.value,
