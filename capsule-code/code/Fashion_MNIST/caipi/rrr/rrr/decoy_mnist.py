...
def _generate_dataset(datadir, fmnist=False):
...
def generate_dataset(cachefile='data/decoy-mnist.npz', fmnist=False):
  print("Loading data")
  if cachefile and os.path.exists(cachefile):
    cache = np.load(cachefile)
    data = tuple([cache[f] for f in sorted(cache.files)])
  else:
    raise ValueError("Error: Should not happen in code ocean")
    data = _generate_dataset(os.path.dirname(cachefile), fmnist)
    if cachefile:
