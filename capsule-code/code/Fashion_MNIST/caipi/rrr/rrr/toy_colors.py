...
def generate_image(label):
    image = np.array(
        [[random_color() for _ in range(imglen)] for __ in range(imglen)],
        dtype=np.uint8,
    )
...
def rule1_score(model, X):
    return np.mean(
        [
            fraction_inside_corners(grad)
            for grad in largest_mag_2d(model.input_gradients(X))
        ]
    )
...
def generate_dataset(N=20000, cachefile="data/toy-colors.npz"):
    if cachefile and os.path.exists(cachefile):
        ...
        data = (
            ...
            np.array([generate_image(y) for y in test_y]),
            train_y,
            test_y,
        )
        if cachefile:
