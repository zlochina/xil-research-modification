...
from ..rrr.toy_colors import generate_dataset, ignore_rule1, ignore_rule2
...
for n in n_vals:
    print(n)
    A = np.array(
        [ignore_rule1 for _ in range(n)]
        + [np.zeros_like(ignore_rule1).astype(bool) for _ in range(len(y) - n)]
    )
