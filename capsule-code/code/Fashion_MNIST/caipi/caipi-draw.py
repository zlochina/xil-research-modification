...
def draw(args):
    ...
    for i_measure in range(perfs.shape[-1]):
        ...
        legend = ax.legend(loc="lower right", fontsize=16, shadow=False)

        fig.savefig(
            args.basename + "_{}.png".format(i_measure),
            bbox_inches="tight",
            pad_inches=0,
        )
    ...
    for i_measure in range(instant_perfs.shape[-1]):
        ...
        fig.savefig(
            args.basename + "_instant_{}.png".format(i_measure),
            bbox_inches="tight",
            pad_inches=0,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "basename", type=str, help="basename of the loss/time PNG plots"
    )
    parser.add_argument(
        "pickles", type=str, nargs="+", help="comma-separated list of pickled results"
    )
    parser.add_argument(
        "--min-pred-val", type=float, default=0, help="minimum pred. score"
    )
    parser.add_argument(
        "--max-pred-val", type=float, default=1.05, help="minimum pred. score"
    )
    parser.add_argument(
        "--min-inst-pred-val",
        type=float,
    ...
    parser.add_argument(
        "--legend", action="store_true", help="whether to draw the legend"
    )
