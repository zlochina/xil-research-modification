...
if n_features == 300:
    ...
    RULE0_BASIS = np.array(
        [
            1.0 if (i, j) in RULE0_COORDS else 0.0
            for i in range(5 * 5)
    ...
    RULE1_BASIS = np.array(
        [
            -1.0 if (i, j) in RULE1_COORDS else 0.0
            for i in range(5 * 5)
            for j in range(i + 1, 5 * 5)
        ]
    )
    ...
    for k in range(n_folds):
        weights = data[k, :, 0, :]
        # (n_iters, n_features)
        alpha, residuals, _, _ = np.linalg.lstsq(DICTIONARY, weights.T, rcond=None)
    ...
    legend = ax.legend(loc="upper right", fontsize=16, shadow=False)
