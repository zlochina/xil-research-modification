...
def main_worker(gpu, args):
    ...
    if args.resume:
        ...
        if os.path.isfile(args.correct_rwr):
            ...
            model.load_state_dict(checkpoint['state_dict'])
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format(args.correct_rwr, checkpoint['epoch']))
        else:
...
def validate(val_loader, model, criterion, epoch, args):
    ...
    if args.evaluate:
        stdout = sys.stdout
        sys.stdout = open(os.path.join(os.path.dirname(args.resume), 'evaluation{}.txt'.format(
            os.path.basename(args.resume).replace("checkpoint", "").replace(".pth.tar", ""))), 'w')
    with torch.no_grad():
        end = time.time()
...
def grad_cam(data_loader, model, args, save_path_suffix):
    ...
    for i, (input, y, _) in enumerate(tqdm(data_loader)):
        ...
        for idx_x, x in enumerate(input):
            ...
            save_filename = os.path.join(os.path.join(save_dir, 'npy'),
