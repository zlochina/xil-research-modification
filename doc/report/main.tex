...
\begin{abstract}
\itodo{Add abstract}
\vtodo{Formal fixes: title, author, abstract. Report class: move all section one level higher: change sections to chapters, subsections to sections, etc. }
\end{abstract}
...
\vtodo{The name cross-entropy is a bit misleading. It is just negative loglikelihood of p(y|x), eq 7. You can remove all entropies...}
...
\begin{figure}
 ...
 \caption{Forward pass of the image \vtodo{is this taken from somewhere? needs to be cited if yes.}}
...
\vtodo{Depends how much needs to be explained for that.}
...
\itodo{Not sure about this statement as it is valid for now and could be changed as there is CounterExamples method (and maybe others). I would rather say that the core of our paper is research of XIL.}\vtodo{Ok, say it the way you like.}
...
\end{align}
\itodo{maybe add l2 regularization theory subsection}\vtodo{It is not really needed, and generally well known, I would not worry much.}
...
to be good too, but what actually happens when we gather such dataset, that may have additional \emph{not necessarily helpful} patterns? \itodo{Maybe we should find some sources to support our argument here?}\vtodo{I think Kersting has more papers on this.}
...
This section describes our plans and ideas for different parts of this bachelor work:
\begin{itemize}
 ...
 For example, we've got a such problem, which requires an expert of the corresponding files to create binary masks, and its time is expensive.
...
\end{itemize}
\itodo{Trust study. I guess, the main goal of XIL, except for correcting the model to be RRR, is building trust. Im thinking, that at some point we would need to do a survey}\vtodo{Survey of what? Poll between users?}
\itodo{Counterfactual explanations suggested by Grad-CAM paper sounds interesting. I think we might use it, but not sure for what.}\vtodo{Explanations have many forms, abductive, counterfactuals, leave it for later...}
...
\itodo{What about EKG dataset experiment? Do we mention it here?} \vtodo{Just mention that we plan to apply the method to a different kind of data, such as EKG.}
...
Although there are different scenarios in the experiment, described in (Section~\ref{sec:plans-improvements}),
...
We have mentioned most of our ideas about which way this research should go on in (Section~\ref{sec:plans-improvements}).
