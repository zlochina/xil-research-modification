...
\date{January 2025}
\author{Vladyslav Zlochevskyi, prof. Ing. Václav Šmídl, Ph.D.}
...
Nowadays deep learning techniques become more advanced and with it comes complexity and non-transparency.
We have seen relatively new concept, Generative AI, to reach results unimaginable for the time of a decade ago,
and those are NLP (Natural Language Processing) or creation of art comparable to the human expert level.
But the core of the models standing behind these inventions is highly non-interpretable - we know they work and correctly in most of the cases,
but do we really know how they work, why their decisions are what they chose?
What if we apply these inventions to the critical infrastructure, where wrong decision could cause a catastrophe?
As of now we do not apply these models for such infrastructure and will not have until we build trust of the
human to the machine.

That is why in this paper we further the research of \emph{Explanatory Interactive Learning} (aka XIL),
concept of which is fundamentally aimed at building trust, in other words,
training the model to be ``right for the right reasons''.
...
\end{itemize}
Relationship between inputs and predictions abstractly speaking is $\hat{y}=f_{\theta}(X)$, where $\theta$ is a vector of learnable parameters.
...
most of the deep learning frameworks suggest usage of \emph{Automatic Differentiation} (aka AD) tool described in~\cite{JMLR:v18:17-468}
and implemented by \emph{PyTorch} framework in~\cite{paszke2017automatic}.
...
\emph{Guided Grad-CAM}~\cite{Selvaraju_2019} is an advanced version of \emph{Grad-CAM} (Subsection~\ref{subsec:gradcam}),
...
\emph{Right for the right reasons}, is a loss function for a differentiable model suggested by~\cite{ross2017right} and modified by~\cite{schramowski2020making}.
...
\emph{Cross-entropy loss} function (Subsection~\ref{subsec:cross-entropy-loss}) modified with class weights.
...
\item Retrain the model using novel approach aka RRRLoss (Subsection~\ref{sec:rrr-theory});
...
so we started from Grad-CAM series of explainers and chose basic Grad-CAM (Section~\ref{subsec:gradcam}).
...
That's why we've decided to use Guided Grad-CAM (Section~\ref{subsec:guided-gradcam}), which provides more detailed
...
The accuracy of the model reached $93\%$, after applying Grad-CAM (Subsection~\ref{subsec:gradcam})
...
to be good too, but what actually happens when we gather such dataset, that may have additional
\emph{not necessarily helpful} pattern, which is greatly described in
\cite[Overfitting occurs when the machine-learning model captures the regularities of data it has been trained on arbitrarily closely,
...
and \emph{same} optimizer, but we use ``right for the right reasons'' loss function (Section~\ref{sec:rrr-theory})
...
This chapter describes our plans and ideas for different parts of this bachelor work:
\begin{itemize}
 \item Codebase: Cam overlay does not work for guided Grad-CAM - we want to fix that
 \item Data scope: we also want to apply described techniques to the different data, such as EKG data of patients.
 \item Scenario: we want to research a scenario, where binary masks are costly to be made.
 In other words we want try out a scenario, where the number of binary masks is so low, that the model would learn without them, but for the most uncertain samples it would query binary mask.
 For example, we've got a such problem, which requires an expert of the corresponding field to create binary masks, and its time is expensive.
 \item Sensitive study: we want to dive deeper in our ``08 MNIST'' experiment and try to answer such question:
 ``what if there is $n$ samples of 8s that no not have the dot?
 How many $n$ I need to get to the accuracy (and right reason) comparable to RRR?''
 \item Art of binary masks: there could be different approaches when creating binary masks for RRR or other methods.
 For example,~\cite[At the other extreme, when A is a matrix of all 1s, it encourages the model to have small gradients
 with respect to its inputs; this can improve generalization on its own Drucker and Le Cun, 1992.]{ross2017right},
\end{itemize}
...
So we have put up the smaller experiment ``08 MNIST'' to see what tools we need to research XIL,
we have written down expectations of this experiment, and we would say, that we reached them.
Although there are different scenarios in the experiment, described in (chapter~\ref{ch:plans-improvements}),
which could be researched further.

\emph{Explainers}.
We have tried out \emph{Grad-CAM} and \emph{Guided Grad-CAM} for our explanations.
For our specific experiment they worked rather well, but as was described before, choice of the explanation method
greatly depends on both the model and our goals, so in further experiments we may need to research other explanation
technique as of right now we do not fully understand the scope of advantages provided by the methodology.

\emph{RRR}.
\emph{Right for the right reasons} cost function has shown good and expected results in the experiment.
Though we haven't researched so-called ``art of creating binary masks'', we've chosen pretty straightforward approach when building them:
we were the ones, who created the ``dots'' on the artificial dataset, and thus it was rather easy to create corresponding binary masks.
But what if we've chosen to penalize gradients in the static region ``where dots may appear'' how that would influence
the final performance?
What we mean by these words is that during the experiment we've come up with different approaches to create binary masks,
and overall this ``art'' shouldn't be ignored.

\emph{Further research}.
We have mentioned most of our ideas about which way this research should go on in (chapter~\ref{ch:plans-improvements}).
