...
\itodo{Explain ADAM}
\vtodo{You can introduce general SGD in general (with preconditioning) and ADAM as a special case how to compute the precondition. But this is not too important. }
...
\begin{align}
 ...
 + \lambda_1 \sum_{l=1}^{L} \sum_{n=1}^{N} \sum_{d=1}^{D} \left( A_{nd} \frac{\delta}{\delta h_{nd}^{l}} \sum_{k=1}^{K} c_k \log (\hat{y}_{nk}) \right)^2 \label{eq:rrr-modified} \\
...
\vtodo{hyperparameters should be listed for sure. Jupyter notebook is optional.}

Thus, we take a standard \emph{MNIST} (\itodo{Maybe add a theory section about this with a brief description})\vtodo{Just a reference, it is known well enough} dataset
...
\vtodo{Make this an extra subsection. Idea with undertraiing is interesting.}
...
\vtodo{yes, this would help a lot.}
